...
import {
  ...
  TARGET_POSITION, CANNIBAL_MIN_IMPRESSIONS, CANNIBAL_MIN_SHARE,
...
function keywordList(site) {
  ...
  return `<ol class="metric-list">${keywords.map((keyword) => {
    ...
    // The page the query ranks (spec item 17), shown on badged rows only — it is
    // the "which page do I open" half of the action, and an unbadged row has no
    // action. Absent when the stored pairs carry no row for this query.
    const where = found && keyword.page
      ? `<a class="query-page" href="${esc(keyword.page)}" target="_blank" rel="noopener" title="${esc(keyword.page)}${keyword.pageCount > 1 ? ` (${keyword.pageCount} pages rank for this query)` : ""}">${esc(pagePath(keyword.page))}</a>`
      : "";
    return `<li class="metric-row ${found ? "flagged" : ""}">
      <div class="metric-name"><span class="truncate" title="${esc(keyword.query)}">${esc(keyword.query)}</span>${found ? `<span class="opportunity-tag ${esc(found.kind)}" title="${esc(why)}">${OPPORTUNITY_LABEL[found.kind]}</span>` : ""}${where}</div>
...
// The two opportunity classes regrouped by page (spec item 17), each under its
// own heading and in its own unit: a page's recoverable-at-current-rank clicks
// and its out-of-reach-until-it-ranks clicks are different questions with
// different fixes, and the two lists are never merged or summed — see
// rankPageOpportunities in opportunities.js. Every figure is a sum over the
// stored (query, page) pairs, which omit Google's anonymized queries, so it is
// labelled "over stored queries" and must never be printed against a
// "Top landing pages (Google Search)" figure for the same page, which counts
// everything Google measured.
const PAGE_CLASS = {
  snippet: { heading: "Rewrite the snippet", metric: "lostClicks", unit: "clicks a window recoverable" },
  rank: { heading: "Strengthen and link", metric: "potentialClicks", unit: "clicks a window out of reach" },
};
function pageOpportunityList(site) {
  const groups = site.pageOpportunities ?? { snippet: [], rank: [] };
  if (!groups.snippet?.length && !groups.rank?.length) return "";
  const block = (kind) => {
    const rows = (groups[kind] ?? []).slice(0, 5);
    if (!rows.length) return "";
    const { heading, metric, unit } = PAGE_CLASS[kind];
    return `<div class="page-class page-class--${kind}"><h4 class="mini-h3"><span class="opportunity-tag ${kind}">${OPPORTUNITY_LABEL[kind]}</span>${heading}</h4>
    <ol class="metric-list">${rows.map((row) => `<li class="metric-row">
      <div class="metric-name"><a class="truncate" href="${esc(row.page)}" target="_blank" rel="noopener" title="${esc(row.page)}">${esc(pagePath(row.page))}</a></div>
      <div class="metric-values"><strong>~${Number(row[metric] || 0).toFixed(1)}</strong><span>${unit} · ${fmt(row.queries)} ${row.queries === 1 ? "query" : "queries"}</span></div>
    </li>`).join("")}</ol></div>`;
  };
  return `<section class="panel pages-panel"><h3><span class="dot search"></span>Pages to fix <span class="panel-note">over stored queries</span></h3>
    <div class="page-classes">${block("snippet")}${block("rank")}</div>
  </section>`;
}

// Queries whose stored impressions are split across two or more of the site's
// own pages (spec item 17). Shares are of the query's STORED impressions, same
// population note as above.
function cannibalizedList(site) {
  const rows = site.cannibalized ?? [];
  if (!rows.length) return "";
  return `<section class="panel pages-panel"><h3><span class="dot traffic"></span>Split queries <span class="panel-note">over stored queries</span></h3>
    <ol class="metric-list">${rows.slice(0, 5).map((row) => `<li class="metric-row split-row">
      <div class="metric-name"><span class="truncate" title="${esc(row.query)}">${esc(row.query)}</span></div>
      <div class="metric-values"><strong>${fmt(row.impressions)} imp</strong><span>${row.pages.length} pages</span></div>
      <div class="split-pages">${row.pages.map((page) =>
        `<a href="${esc(page.page)}" target="_blank" rel="noopener" title="${esc(page.page)}">${esc(pagePath(page.page))}</a> <small>${pct(page.share, 0)} · pos ${Number(page.position || 0).toFixed(1)}</small>`).join("<span class=\"split-sep\"> vs </span>")}</div>
    </li>`).join("")}</ol>
  </section>`;
}
...
function siteCard(site, index, periodDays) {
  ...
  return `<section class="card ${site.zoneSourced ? "card--zone" : ""} ${!site.visits && !hasDetails ? "empty" : ""}" aria-labelledby="${id}">
    ...
    ${hasDetails ? (site.zoneSourced ? `<details class="detail" open data-card-index="${index}">
      ...
      ${pageOpportunityList(site)}
      ${cannibalizedList(site)}
...
// "AI referrals" tile: RUM sessions whose referer is an AI chat/answer engine
// (AI_ANSWER_ENGINES in config.js, matched on the stored referer host so a newly
// listed engine counts retroactively). A floor, and labelled as one: most AI
// surfaces send no Referer, and AI Overviews / Copilot in Bing arrive as
...
// One tile for search CTR and position (they answer one question: is the
// estate ranking well, and converting the rank it has?).
//
// Populations, stated because this tile has been wrong about them before:
// - Headline CTR is Google + Bing whole-corpus clicks over impressions
//   (daily_search_summary + daily_bing_summary). Same shape on both sides, so it
//   sums honestly, and it matches the Search clicks/impressions tiles beside it.
// - Headline position is GOOGLE's median over its stored top queries. Bing's
//   median is over Bing's own stored query rows (weekly, a different top-N) and
//   is printed beside it, never pooled into one median.
// - The expected-CTR comparator and the 14-snapshot mean are Google-only and sit
//   on the Google line, so a combined headline is never compared with a
//   Google-only figure.
function searchQualityTile({ totals, trend, hasBingSite, bingClicksTotal, bingImpressionsTotal,
  expected, opportunityNote, rolling }) {
  const clicks = totals.gscClicks + (hasBingSite ? bingClicksTotal : 0);
  const impressions = totals.gscImpressions + (hasBingSite ? bingImpressionsTotal : 0);
...
export function renderDashboard(data) {
  ...
  const stats = [
    ...
    aiTile(totals.ai ?? {}, trend, totals, data.periodDays),
  ...
  if (totals.searchDataDomains) {
    ...
    stats.push(
      ...
      searchQualityTile({ totals, trend, hasBingSite, bingClicksTotal, bingImpressionsTotal,
        expected, opportunityNote, rolling }),
    );
  }
...
<div class="wrap">
  ...
  <footer>
    ...
    <div><b>Each badged query names the page it ranks, and <i>Pages to fix</i> regroups both classes by page</b> — from the stored (query, page) pairs, each class summed in its own unit and never added to the other. Those pairs omit the queries Google anonymizes, so a page's figures there are <i>over stored queries</i> and are not the same population as its row under <i>Top landing pages (Google Search)</i>, which counts everything Google measured. A <b>split query</b> is one with at least ${fmt(CANNIBAL_MIN_IMPRESSIONS)} stored impressions where two or more of the site's own pages each hold at least ${pct(CANNIBAL_MIN_SHARE, 0)} of them: Google is choosing between them, so the fix is to pick one and consolidate, not to strengthen either.</div>
    ...
    ${hasBingSite ? `<div><b>Bing Search</b> is pulled from Bing Webmaster Tools, a separate account-wide API key rather than GSC's OAuth. Per site, it stays on its own card row, never merged into that site's Google figures — a different engine's audience, measured by a different tool. Only the estate-wide tiles above add the two together: "Search clicks", "Search impressions", and the headline CTR on the <b>Search CTR · position</b> tile, all whole-corpus clicks over impressions on both engines. Position is never pooled: the headline is Google's median, Bing's median over its own stored query rows sits on its own line, and the expected-CTR comparison and 14-snapshot mean are Google-only and say so. The site-wide summary (clicks, impressions, CTR) updates daily; per-query rows update weekly, so a query list can be several days further behind than the summary beside it — each carries its own window. Bing reports two positions per query (where clicks landed, and where every impression landed) rather than GSC's one, shown as reported rather than averaged into a single figure. Only sites with a verified Bing Webmaster Tools property (<code>bing</code> in <code>src/config.js</code>) are pulled; page-level Bing data is not collected, to stay within the Worker's per-invocation subrequest budget.</div>` : ""}
